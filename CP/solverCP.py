...
# Build the CP model
def build_model(d, rtime, fixed, ss, log_output):
    ...
    solver.parameters.log_search_progress = log_output
...
def solve_CP(d, rtime, log_output):
    print("Solving a constraint programming model of the Happy Colourings problem ...")
    fixed = {}
    for i in range(d.vertices):
        for j in range(d.colours):
            fixed[i,j] = 1 # Initially no colours for any vertex is allowed

    ss = []
    for i in range(d.vertices):
        ss.append(-1)

    obj = build_model(d, rtime, fixed, ss, log_output)

# Solve the happy colouring problem CMSA (CP solver)
def solve_CMSA(d, rtime, stime, log_output):
    ...
    while time.time() - start_time < rtime:
        ...
        t = time.time()
        sol = build_model(d, stime, fixed_vars, x_start, log_output)
...
ap.add_argument('--meth', default='cp', type=str,
...
ap.add_argument('--stime', default=10, type=str,
    help='''Solver time?''')
ap.add_argument('--lo', default=10, type=str,
    help='''Log output?''')
...
# Solve a CP model of the HC problem
    ...
    solve_CP(d, rtime, log_output)
#solve CMSA
else:
    solve_CMSA(d, rtime, stime, log_output)
